...
var peer_server = PeerServer({port: 8888, path: '/'});
/* peerJS event listener for 'open' does not really work, if there is an issue
opening, additional errors will appear.*/
console.log("**2 peerJS server initialized");
...
UDP_server.on('listening', function () {
    ...
    console.log('**1 UDP Server listening on ' + address.address + ":" + address.port);
});
...
wss.on('connection', function connection(ws) {
    ...
    if(myGlobalSocket)
        myGlobalSocket.emit("AndroidStatus",1);
  ...
  ws.on('close', function connection(ws) {
    ...
    if(myGlobalSocket != undefined)
        myGlobalSocket.emit("AndroidStatus",0);
  });
...
peer_server.on('disconnect', function(id) {
       console.log("peerClient: " + id + " disconnected!");
 });
/*
/*
/*
...
port.on('open', function() {
    ...
    //     }
    // });
    console.log("**3 Serial Communication Ready!");
    serialConnected = true;
